...
function Product({ which }: { which: ProductKind }) {
  if (which === "ro")
    return (
      <svg className="product" width="300" height="320" viewBox="0 0 300 320" fill="none" aria-hidden="true">
  ...
  if (which === "compact")
    return (
      <svg className="product" width="280" height="320" viewBox="0 0 280 320" fill="none" aria-hidden="true">
  ...
  return (
    <svg className="product" width="300" height="330" viewBox="0 0 300 330" fill="none" aria-hidden="true">
...
export default function HeroBanners() {
  ...
  return (
    ...
    >
      ...
      <div className="track">
        <article className="slide active" data-accent={slide.accent} key={slide.key}>
          <div className="content">
            ...
            <div className="chips">
              {slide.chips.map((c) => (
                <span className="chip" key={c}><Check />{c}</span>
              ))}
            </div>
            <div className="ctas">
              ...
              <Link className="btn btn-ghost" href={slide.ghost.href}>{slide.ghost.label}</Link>
            </div>
          </div>
      ...
      <div className="controls">
        <div className="tabs" role="tablist" aria-label="Перемикання банерів">
          {SLIDES.map((s, k) => (
            <button
              key={s.key}
        ...
        <div className="arrows">
          <button className="arrow" aria-label="Попередній банер" onClick={() => go(i - 1)}>
            <svg fill="none" stroke="currentColor" strokeWidth="2.4" viewBox="0 0 24 24">
              <path d="M15 18l-6-6 6-6" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <button className="arrow" aria-label="Наступний банер" onClick={() => go(i + 1)}>
            <svg fill="none" stroke="currentColor" strokeWidth="2.4" viewBox="0 0 24 24">
      ...
      <style jsx>{`
        .hero {
          ...
          box-shadow: 0 30px 60px -30px rgba(11, 31, 51, 0.18);
          isolation: isolate;
        ...
        .wave { position: absolute; left: 0; right: 0; bottom: 0; z-index: 0; height: 120px; opacity: 0.5; pointer-events: none; }
        ...
        .slide {
          ...
          animation: slideIn 0.7s cubic-bezier(0.22, 0.8, 0.3, 1) both;
        }
        @keyframes slideIn { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
        .slide[data-accent="green"] { --accent: var(--green); --accent-dark: var(--green-dark); }
        .slide[data-accent="teal"] { --accent: var(--teal); --accent-dark: #007a9c; }
        .slide[data-accent="blue"] { --accent: var(--blue); --accent-dark: var(--primary-900, #003b73); }
        .eyebrow {
        ...
        h1 :global(em) { font-style: normal; color: var(--accent-dark); }
        .sub { font-size: 17px; line-height: 1.65; color: var(--muted); max-width: 47ch; margin-bottom: 28px; }
        ...
        .chip :global(svg) { width: 16px; height: 16px; stroke: var(--accent-dark); }
        ...
        .visual { position: relative; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 380px; }
        .halo {
          position: absolute; width: 400px; height: 400px; border-radius: 50%;
          background: radial-gradient(circle, color-mix(in srgb, var(--accent) 18%, transparent) 0%, transparent 70%);
          animation: pulse 6s ease-in-out infinite;
        }
        @keyframes pulse { 0%, 100% { transform: scale(1); opacity: 0.8; } 50% { transform: scale(1.07); opacity: 1; } }
        .rings { position: absolute; inset: 0; display: grid; place-items: center; pointer-events: none; }
        .rings i { position: absolute; border-radius: 50%; border: 1px solid color-mix(in srgb, var(--accent) 30%, transparent); }
        .rings i:nth-child(1) { width: 300px; height: 300px; }
        .rings i:nth-child(2) { width: 400px; height: 400px; border-color: color-mix(in srgb, var(--accent) 18%, transparent); }
        .rings i:nth-child(3) { width: 500px; height: 500px; border-color: color-mix(in srgb, var(--accent) 9%, transparent); }
        .visual :global(.product) { position: relative; z-index: 1; filter: drop-shadow(0 22px 34px rgba(11, 31, 51, 0.18)); }
        .stats {
        ...
        .stat { flex: 1; text-align: center; padding: 10px 18px; position: relative; }
        .stat + .stat::before { content: ""; position: absolute; left: 0; top: 18%; bottom: 18%; width: 1px; background: var(--line); }
        .stat b { display: block; font-family: var(--font, "Unbounded", sans-serif); font-size: 18px; font-weight: 700; color: var(--accent-dark); white-space: nowrap; }
        .stat span { font-size: 12px; color: var(--muted); font-weight: 700; }
        .controls { position: absolute; left: 72px; right: 72px; bottom: 26px; z-index: 3; display: flex; align-items: center; gap: 20px; }
        ...
        .tab:focus-visible { outline: 2px solid var(--green); outline-offset: 2px; border-radius: 8px; }
        .tab::after { content: ""; position: absolute; left: 0; bottom: 0; height: 3px; width: 100%; background: var(--line); border-radius: 99px; }
        .tab .bar { position: absolute; left: 0; bottom: 0; height: 3px; width: 0; background: var(--green); border-radius: 99px; z-index: 1; }
        .tab.active { color: var(--ink); }
        .tab.active .bar { animation: fill ${DUR}ms linear forwards; }
        ...
        .arrow:hover { border-color: var(--green); color: var(--green-dark); transform: scale(1.06); }
        .arrow:focus-visible { outline: 2px solid var(--green); outline-offset: 2px; }
        .arrow :global(svg) { width: 18px; height: 18px; }
        @media (max-width: 980px) {
          .slide { grid-template-columns: 1fr; padding: 48px 36px 112px; min-height: auto; }
          .rings i:nth-child(3) { width: 420px; height: 420px; }
          .visual { min-height: 320px; order: -1; }
          .controls { left: 36px; right: 36px; }
        }
        @media (max-width: 560px) {
          .slide { padding: 38px 22px 132px; }
          .controls { left: 22px; right: 22px; flex-direction: column-reverse; align-items: stretch; gap: 12px; }
          ...
          .visual :global(.product) { max-width: 200px; height: auto; }
          .rings i:nth-child(3) { display: none; }
          .stat b { font-size: 15px; }
          .stat { padding: 8px 10px; }
        }
        @media (prefers-reduced-motion: reduce) {
          .blobs, .halo { animation: none; }
          .slide { animation: none; }
          .tab.active .bar { animation: none; width: 100%; }
        }
      `}</style>
    </section>
  );
